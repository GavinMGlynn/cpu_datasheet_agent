...
| D78 | 2026-09-13 | CI runs on GitHub-hosted runners, not the self-hosted one, and installs what it needs. The browser tests run inside `mcr.microsoft.com/playwright`, pinned to the version in `package.json` and checked against it; the screenshot baselines are taken there and committed under `test/e2e/__screenshots__/playwright-noble/`. Supersedes D21 and D71. | The repository is public. A pull request from a fork carries its own workflow code, and a self-hosted runner would run it on a private VM on a home network — GitHub says plainly not to pair the two. Moving to hosted runners costs the "the build machine is the target machine" argument, so the screenshots needed a platform anyone can stand on instead: a pinned container has the fonts, the rasteriser and the browser in it, which is a better answer than one machine nobody else has. | active |
| D79 | 2026-09-13 | The project is published under the GNU General Public License v3.0 or later, with a section 7 additional permission for combining it with the Claude Agent SDK and the Claude Code binaries it carries. `LICENSE` holds the licence; the README holds the statement. | It was written to learn from and is worth more read than kept, and copyleft keeps a fork that learns from it readable too. Version 3 rather than 2 because TypeScript and Playwright are Apache-2.0, which GPLv2 cannot take. The additional permission is the honest part: the agent harness is Anthropic's proprietary software, so a combined work needs that said out loud rather than assumed. | active |
| D80 | 2026-09-13 | The version in `package.json` is the one source of truth: `src/version.ts` reads it, and the site, the health endpoint and the packaged archive all report that. The first public release is `v0.1.0-beta1`. | The site said "Version 1.0.0" because a default in `create.ts` said so, which was true of nothing. A version people can see is a claim about which build they are looking at, and a second copy of it in the source is a second copy to forget. | active |
...
Every job runs on a GitHub-hosted runner (D78): environment, check, build,
browser tests, release. The browser job runs inside the pinned
`mcr.microsoft.com/playwright` image, whose tag is checked against
`package.json` before anything else starts; the twelve screenshot baselines
are taken there and committed under
`test/e2e/__screenshots__/playwright-noble/`, because a screenshot baseline is
a photograph of one machine's fonts and a container is a machine anyone can
have. The self-hosted runner is retired: it cannot be pointed at a public
repository, where a fork's pull request brings its own workflow.
