...
- [x] 19H.1 `src/web/README.md`: the public API, the errors, the invariants,
      and the security model.
- [x] 19H.2 `docs/ARCHITECTURE.md`: the web layer, its boundary with the
...
A token in an address is not authentication: it is in the history, in the
referrer, in whatever copies the link, and it dies when the server restarts.
This module replaces it with accounts, sessions and roles (D75, D76).
